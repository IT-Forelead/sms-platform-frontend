  ...
  <div class="p-3">
    <div class="flex items-center justify-between">
      <div class="relative">
        ...
        <input v-model="search" type="search" name="search" class="bg-white text-gray-500 py-1.5 text-lg rounded-lg pl-12 focus:outline-none outline-none border-0 dark:bg-gray-800 dark:text-gray-400 dark:placeholder:text-gray-500" placeholder="Raqam bo'yicha izlash..." autocomplete="off" />
      </div>
      <div class="relative pr-3" x-data="{sort: false}">
        <div x-on:click="sort = true" class="bg-white rounded-xl px-5 p-2 ml-3 text-gray-500 font-semibold hover:bg-gray-200 cursor-pointer dark:bg-gray-800 dark:text-gray-400">
          <i class="fa fa-filter"></i> {{ sortBy === '' ? 'Saralash' : changeSort(sortBy) }} <i @click="clearSort()" class="hover:text-red-500 ml-1 cursor-pointer fa" :class="{ 'fa-times': sortBy !== '' }"></i>
        </div>
        <div x-show="sort" x-on:click.outside="sort = false" class="dropdown absolute right-0 top-12 z-10 bg-white border rounded shadow w-44 dark:bg-gray-800 dark:border-gray-600">
          <ul class="text-sm text-gray-700 dark:text-gray-200">
            <li class="border-b border-dotted cursor-pointer dark:border-gray-600">
              <a x-on:click="sort = false" @click="sortByFunc('soFarBirthday')" class="block px-4 py-2 text-gray-700 hover:bg-gray-100 dark:text-gray-400 dark:hover:bg-gray-700">
                <i class="fa-solid fa-users mr-1"></i> Tug'ilgan kun
              </a>
            </li>
            <li class="border-b border-dotted cursor-pointer dark:border-gray-600">
              <a x-on:click="sort = false" @click="sortByFunc('male')" class="block px-4 py-2 text-gray-700 hover:bg-gray-100 dark:text-gray-400 dark:hover:bg-gray-700">
                <i class="fa-solid fa-mars mr-1"></i> Erkaklar
              </a>
            </li>
            <li class="cursor-pointer">
              <a x-on:click="sort = false" @click="sortByFunc('female')" class="block px-4 py-2 text-gray-700 hover:bg-gray-100 dark:text-gray-400 dark:hover:bg-gray-700">
                <i class="fa-solid fa-venus mr-1"></i> Ayollar
              </a>
            </li>
          </ul>
        </div>
    ...
    <div class="mt-3 pr-3 rounded-lg relative custom-height overflow-y-auto">
      <div v-if="showContent">
        <div x-data="{open: false}" v-for="(contact, index) in contacts" :key="index" class="relative bg-white px-5 py-2 my-2 flex items-center rounded-lg hover:bg-violet-100 dark:bg-gray-800 dark:hover:bg-gray-700">
          ...
          <div class="ml-4">
            <p class="text-2xl font-semibold m-0 p-0 dark:text-gray-300">{{ phoneStyle(contact.phone) }}</p>
            <p class="text-lg text-gray-500 -mt-0.5 font-semibold dark:text-gray-400">{{ contact.firstName + ' ' + contact.lastName }}</p>
          </div>
          <i x-on:click="open = true" class="fa-solid fa-ellipsis-vertical p-1.5 px-3 absolute cursor-pointer hover:text-gray-500 top-1 right-1 rounded-full dark:text-gray-300 dark:hover:text-gray-300"></i>
          <div x-show="open" x-on:click.outside="open = false" class="dropdown-content-for-edit-contact absolute right-1 top-8 z-10 bg-white border dark:bg-gray-800 dark:border-gray-600 rounded shadow w-44">
            <ul class="text-sm">
              <li class="border-b border-dotted dark:border-gray-600">
                <a @click="openEditModal(contact)" class="block px-4 py-2 text-gray-700 hover:bg-gray-100 dark:text-gray-300 dark:hover:bg-gray-700"><i class="fa-solid fa-user-pen mr-1"></i> Taxrirlash</a>
              </li>
              <li>
                <a @click="deleteContact(contact.id)" class="block px-4 py-2 text-gray-700 hover:bg-gray-100 dark:text-gray-300 dark:hover:bg-gray-700"><i class="fa-solid fa-trash mr-1"></i> O'chirish</a>
              </li>
            </ul>
          </div>
        </div>
      </div>
      <div v-else class="rounded-lg text-lg bg-white px-5 py-10 dark:bg-gray-800">
  ...
  <div v-show="isShowModal" id="edit-modal" tabindex="-1" class="overflow-y-auto w-full overlay overflow-x-hidden fixed top-0 right-0 left-0 z-40 flex items-center justify-center md:inset-0 h-modal md:h-full">
    <div class="relative p-4 w-full max-w-2xl h-full md:h-auto text-gray-800">
      <div class="relative bg-white rounded-lg shadow-lg dark:bg-gray-800 z-50">
        ...
        <h3 class="text-2xl font-extrabold py-5 ml-5 dark:text-gray-300">Kontaktni taxrirlash</h3>
